...
public class EndPanel : MonoBehaviour
{
    ...
    private string questionText = "언제까지의" + System.Environment.NewLine + "분석 결과가" + System.Environment.NewLine + "보고 싶나요?";
    ...
    public void ErrorCheck()
    {
        ...
        int startYear = UIManager.instance.StartYear;
        if (yearInput.text.Length == 4 && startYear <= year && year <= 2020)
        {
            ChangeErrorLog("");
            isValid = true;
            endYear = year;
        ...
        else if (1990 <= year && year < startYear)
        {
            ChangeErrorLog(startYear + "보다 큰 숫자를 입력해주세요.");
            isValid = false;
            return;
        }
        else
        {
            ChangeErrorLog(startYear + "~2020 사이의 분석만 가능합니다.");
            isValid = false;
            return;
        }
    }
    ...
    {
        UIManager.instance.EndToLoadingPanel(endYear);
    }
}
